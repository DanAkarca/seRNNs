...
function output = run_se_swc_iv_generative_models(threshold,network,epoch,el,eu,gl,gu);
...
% change directory
cd('/imaging/shared/users/ja02/CBUActors/SpatialRNNs/Results/mazeGenI_SE1_sWc_mtIV_1k');
% set the savedir
sdir = '/imaging/astle/users/da04/PhD/ann_jascha/data/se_swc_iv_generative_models';
...
for net = 1:nnet;
    ...
    % display
    disp(sprintf('se_swc_4_1k network %g threhsolded at %g',net-1,threshold));
end
...
for model = 1:nmodels;
    % print text
    disp(sprintf('running se _swc_4_1k_rnn_thr%g_net%g_epo%g model %s...',threshold,network,epoch,modeltypes(model)));
    % run the model
...
cd(sdir);
% save file
save(sprintf('se_swc_4_1k_thr%g_net%g_epo%g_generative_model.mat',threshold,network,epoch),'output','-v7.3');
end